                               ...
                               const char *soc_type)
{
    ...
    //rpi_gpio = qdev_create(NULL, "bcm2835-gpio");
    // qdev_init_nofail(rpi_gpio);
    // printf("DEBUG: before link rpi_gpio - interrupt controller\n");
    // // sysbus_connect_irq(SYS_BUS_DEVICE(rpi_gpio), 0,
    // //         qdev_get_gpio_in_named(DEVICE(&(BCM2836(s->soc)->control)), BCM2835_IC_GPU_IRQ, 0));
    
    // printf("DEBUG: before link rpi_gpio - interrupt controller\n");
    // sysbus_mmio_map(SYS_BUS_DEVICE(rpi_gpio), 0, 0x3F200000);
